...
function gerarGrafico(nome, qtd, qtdlida) {
    ...
    let lidoPercent = Math.floor((qtdlida * 100) / qtd)
    let totalPercent = 100 - lidoPercent
    var myChart = new Chart(ctx, {
        type: 'doughnut',
        data: {
            labels: ['Total Livro', 'Lido'],
            datasets: [{
                data: [totalPercent, lidoPercent],
...
listarBtn.addEventListener('click', e => {
    ...
    for (let j = 0; j < array.length; j++) {
        ...
        td2.appendChild(document.createTextNode(j+1))
        tr.appendChild(td2)
